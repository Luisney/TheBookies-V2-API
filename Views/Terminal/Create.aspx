...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <fieldset>
        ...
        <p>
            <%= this.Select( PageModel => PageModel.CompanyId ).Options( Model.CompanyList ).FirstOption( "Selecciona una" ).Label( "Consorcio:" )%>
            <%= this.ValidationMessage( PageModel => PageModel.CompanyId )%>
        </p>
        <p>
            <%= this.Select( PageModel => PageModel.SportBookId ).Label( "Banca:" )%>
            <%= this.ValidationMessage( PageModel => PageModel.SportBookId )%>
        </p>
        <p>
            <%= this.Select( PageModel => PageModel.StoringCenterId ).Label( "Centro de acopio:" )%>
